    ...
    <Card onClick={() => router.push(`/equipment/${equipment._id}`)} cursor-pointer className="group flex h-full min-h-0 flex-col overflow-hidden border-0 rounded-2xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] md:rounded-[2rem]">
      <CardHeader className="shrink-0 p-0">
        <div className="relative h-40 md:h-56 overflow-hidden">
          ...
          />
          <Badge className={`absolute top-2.5 right-2.5 md:top-4 md:right-4 px-3 py-1 md:px-4 md:py-1.5 rounded-full text-xs md:text-sm font-black border-0 shadow-lg ${statusColor}`}>
            {equipment.status}
          </Badge>
      ...
      <CardContent className="flex flex-1 flex-col gap-3 p-4 pb-5 text-right md:gap-4 md:p-6 md:pb-6">
