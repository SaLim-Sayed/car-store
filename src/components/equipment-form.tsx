...
export function EquipmentForm({
  ...
  const handleShowroomChange = (showroom_id: string) => {
    if (!showroom_id) {
      set("showroom", "")
      return
    }
    const showroom = showrooms.find((s: any) => s._id === showroom_id)
    if (showroom) {
      onChange({
        ...form,
        showroom: showroom_id,
        location: showroom.address || form.location,
        phone: showroom.phone || form.phone,
        locationLink: showroom.locationLink || form.locationLink,
      })
    }
  }
            ...
            {showrooms.length > 0 && (
              <div className="space-y-2.5">
                ...
                <ShowroomAutocomplete
                  listId="equipment-form-showrooms"
                  showrooms={showrooms}
                  value={form.showroom || ""}
