      ...
      <main className="container mx-auto px-4 py-8 md:py-24">
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[...Array(4)].map((_, index) => (
                <Card
                  key={index}
                  className="border-0 shadow-lg rounded-2xl md:rounded-[2rem] bg-white overflow-hidden"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredNews.map((item: any) => (
                <Card
                  key={item._id}
                  className="border-0 shadow-xl rounded-2xl md:rounded-[2rem] bg-white overflow-hidden group hover:shadow-2xl transition-all duration-500 flex flex-col"
                >
                  {item.image && (
                    <div className="h-40 md:h-52 overflow-hidden relative">
                      ...
                      <div className="absolute top-3 right-3 md:top-4 md:right-4">
                        <Badge className="bg-white/90 backdrop-blur-md text-primary border-0 rounded-full px-3 py-1 font-black shadow-sm">
                          {item.category}
                        </Badge>
                      </div>
                    </div>
          ...
          {!isLoading && filteredNews.length === 0 && (
            <Card className="border-0 shadow-2xl rounded-[2rem] p-16 text-center bg-white">
