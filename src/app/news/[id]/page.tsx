...
export default function NewsDetailPage() {
  ...
  const shareToTwitter = () => {
    const url = `https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(news?.title || "")}`;
    window.open(url, "_blank", "width=600,height=400");
  };
              ...
              <Card className="border-0 shadow-none rounded-2xl md:rounded-[2.5rem] bg-white overflow-hidden">
                ...
                <CardContent className="p-5 md:p-8 pt-0 flex flex-row md:flex-col gap-3">
                  <Button
                    variant="outline"
                    onClick={copyLink}
                    className="flex-1 md:flex-none h-11 md:h-14 rounded-xl md:rounded-2xl border-2 font-bold justify-start gap-2 md:gap-4 text-sm md:text-base"
                  >
                  ...
                  <Button
                    ...
                    className="flex-1 md:flex-none h-11 md:h-14 rounded-xl md:rounded-2xl border-2 font-bold justify-start gap-2 md:gap-4 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 text-sm md:text-base"
                  >
              ...
              <Card className="border-0 shadow-none rounded-2xl md:rounded-[2.5rem] bg-primary text-white overflow-hidden">
                <CardContent className="p-5 md:p-8 space-y-4 md:space-y-6">
                  ...
                  <p className="text-sm md:text-base font-bold opacity-80">
                    كن أول من يعلم بجديد السيارات والعروض الحصرية
                  </p>
                  <Button asChild className="w-full h-11 md:h-14 rounded-xl md:rounded-2xl bg-white text-primary hover:bg-white/90 font-black text-base md:text-lg">
                    <a href={getWhatsAppUrl("مرحباً، أرغب في الاشتراك بنشرتكم الإخبارية لمعرفة أحدث العروض")} target="_blank" rel="noopener noreferrer">
                      اشترك عبر واتساب
                    </a>
                  </Button>
                </CardContent>
